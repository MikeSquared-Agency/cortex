...
fn make_manual(created_by: &str) -> EdgeProvenance {
    EdgeProvenance::Manual { created_by: created_by.to_string() }
}
...
fn test_storage_persistence() {
    ...
    let storage = RedbStorage::open(&db_path).unwrap();
    let node = storage.get_node(node_id).unwrap().expect("Node should survive reopen");
...
fn test_graph_bfs_traversal() {
    ...
    assert!(subgraph.nodes.contains_key(&node_b.id), "B should be reachable from A");
    assert!(subgraph.nodes.contains_key(&node_c.id), "C should be reachable from A");
...
fn test_graph_depth_limit() {
    ...
    assert!(!subgraph.nodes.contains_key(&c.id), "C should be beyond depth limit");
}
...
fn test_graph_neighborhood() {
    ...
    assert!(!subgraph.nodes.contains_key(&far.id), "Far should be beyond depth 1");
...
fn test_vector_index_rebuild() {
    ...
    let vector_index = Arc::new(StdRwLock::new(HnswIndex::new(embedding_service.dimension())));
    ...
    for i in 0..5 {
        ...
        storage.put_node(&node).unwrap();
        vector_index.write().unwrap().insert(node.id, &embedding).unwrap();
    }
    ...
    let results = vector_index.read().unwrap().search(&query, 5, None).unwrap();
...
fn test_similarity_search_returns_relevant_results() {
    ...
    let topics = vec![
        ...
        ("Python scripting", "Python is great for scripting and data science"),
        ("Machine learning", "ML models learn patterns from training data"),
    ];
    ...
    let query_emb = embedding_service.embed("Rust systems programming safety").unwrap();
    let results = vector_index.read().unwrap().search(&query_emb, 3, None).unwrap();
...
fn test_auto_linker_creates_similarity_link() {
    ...
    let mut auto_linker = AutoLinker::new(
        storage.clone(), graph_engine, vector_index.clone(), embedding_service.clone(), config,
    ).unwrap();
...
fn test_auto_linker_metrics_update_after_cycle() {
    ...
    let mut auto_linker = AutoLinker::new(
        storage.clone(), graph_engine, vector_index, embedding_service, AutoLinkerConfig::default(),
    ).unwrap();

    assert_eq!(auto_linker.metrics().cycles, 0);
    auto_linker.run_cycle().unwrap();
    assert_eq!(auto_linker.metrics().cycles, 1, "Cycle count should increment");
}
...
fn test_edge_decay_preserves_recent_auto_edges() {
    ...
    assert_eq!(pruned, 0, "Recently created auto-edges should not be pruned immediately");
    assert_eq!(deleted, 0, "Recently created auto-edges should not be deleted immediately");

    // Edge should still exist
    let retrieved = storage.get_edge(edge.id).unwrap();
    assert!(retrieved.is_some(), "Edge should still exist after decay pass");
}
...
fn test_edge_decay_exempts_manual_edges() {
    ...
    assert_eq!(deleted, 0, "Manual edges should be exempt from decay deletion");
...
fn test_hybrid_search_finds_relevant_nodes() {
    ...
    assert!(results[0].vector_score > 0.0, "Top result should have a positive vector score");
}
