use anyhow::Result;
use cortex_proto::*;
use crate::cli::{TraverseArgs, PathArgs, grpc_connect};
...
pub async fn run(args: TraverseArgs, server: &str) -> Result<()> {
    ...
    if args.format == "json" {
        ...
        println!("Subgraph: {} nodes, {} edges (visited: {}{})",
            resp.nodes.len(), resp.edges.len(), resp.visited_count,
        ...
        if !resp.nodes.is_empty() {
            ...
            for node in &resp.nodes {
                ...
                println!("{}[{}] {} — {} ({})", indent, depth, node.id, node.title, node.kind);
            }
        }
        ...
        if !resp.edges.is_empty() {
            ...
            for edge in &resp.edges {
                println!("  {} --[{}]--> {} ({:.2})",
                    &edge.from_id[..8], edge.relation, &edge.to_id[..8], edge.weight);
            }
        }
    }
...
pub async fn run_path(args: PathArgs, server: &str) -> Result<()> {
    ...
    if args.format == "json" {
        let paths: Vec<_> = resp.paths.iter().map(|p| serde_json::json!({
            "node_ids": p.node_ids,
