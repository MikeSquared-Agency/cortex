...
impl IngestAdapter for NatsAdapter {
    ...
    async fn subscribe(&self) -> Result<BoxStream<'static, IngestEvent>> {
        let client: Client = async_nats::connect(&self.url)
            .await
            .map_err(|e| CortexError::Validation(format!("NATS connect to {} failed: {}", self.url, e)))?;
        ...
        for subject in &self.subjects {
            let sub = client
                ...
                .await
                .map_err(|e| CortexError::Validation(format!("NATS subscribe to '{}' failed: {}", subject, e)))?;
